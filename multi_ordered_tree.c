...
typedef struct tree_node_s
{
  ...
  char niss[MAX_TELEPHONE_NUMBER_SIZE - 1];              // index 2 data item
...
void list(tree_node_t *link, int tree_index)
{
  ...
  printf("NISS --- %s\n", link->niss);                          // imprime a info niss da link
...
void list_find(tree_node_t *link, int tree_index, char *search,int option)
{
  ...
  int comp;
  if (option==1){
    comp = strcmp (link->niss,search);
  }
  else if(option==0){
      comp = strcmp (link->zip_code,search);
  }
  else{
    char copiedname[strlen(link->name)];
    strcpy(copiedname,link->name);
    char test[strlen(search)];
...
int main(int argc, char **argv)
{
  ...
  if (argc < 3)
  {
    ...
    fprintf(stderr, "  -niss [N]             # list the  person who has the given niss code\n");
    return 1;
  }
